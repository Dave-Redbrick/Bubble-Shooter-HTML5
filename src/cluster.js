    ...
    if (this.game.animationState === 0) {
      ...
      this.game.cluster.sort((a, b) => {
        ...
        const distA = Math.sqrt(
          Math.pow(coordA.tileX + this.game.levelData.tileWidth / 2 - hitPos.x, 2) +
          Math.pow(coordA.tileY + this.game.levelData.tileHeight / 2 - hitPos.y, 2)
        );
        const distB = Math.sqrt(
          Math.pow(coordB.tileX + this.game.levelData.tileWidth / 2 - hitPos.x, 2) +
          Math.pow(coordB.tileY + this.game.levelData.tileHeight / 2 - hitPos.y, 2)
        );
      ...
      // 버블 개당 10점으로 점수 계산
      this.game.updateScore(this.game.cluster.length * 10);
      ...
      for (const tile of this.game.cluster) {
        if (tile.type >= 0) {
          ...
          if (this.game.animationTime >= tile.popTime && !tile.popTriggered) {
            ...
            // 강화된 파티클 효과
            this.game.particles.createChainPopEffect(centerX, centerY, color, 0);
      ...
      for (const cluster of this.game.floatingClusters) {
        for (const tile of cluster) {
          if (tile.type >= 0) {
            ...
            if (tile.alpha === 0 || 
                (tile.y * levelData.rowHeight + tile.shift > (levelData.rows - 1) * levelData.rowHeight + levelData.tileHeight)) {
              tile.type = -1;
              tile.shift = 0;
              tile.alpha = 1;
            }
  ...
  findFloatingClusters() {
    ...
    for (let i = 0; i < levelData.columns; i++) {
      for (let j = 0; j < levelData.rows; j++) {
        ...
        if (!tile.processed) {
          const foundCluster = this.game.physics.findCluster(i, j, false, false, true);
